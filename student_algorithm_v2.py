...
class TradingBot:
    ...
    def _strategy_normal_market(self, bid: float, ask: float, mid: float, regime: str) -> Optional[Dict]:
        """
        Classic Market Making: Place orders slightly inside or at the best bid/ask.
        Goal: Capture the spread.
        """
        # Alternate between buying and selling based on inventory skew
        # If we have too many, focus on selling. If we have too few, focus on buying.
        qty = 100
        
        if self.inventory > 500: # Lean towards selling
            return self._create_order("SELL", ask, qty)
        elif self.inventory < -500: # Lean towards buying
            return self._create_order("BUY", bid, qty)
        else:
            # Neutral: flip a coin or alternate to provide liquidity on both sides
    ...
    def _strategy_stressed_market(self, bid: float, ask: float, mid: float, regime: str) -> Optional[Dict]:
        """
        Goofy Gambler 6.1: The Sniper. 
        Fixed unpacking error and optimized for long-duration play.
        """
        import random

        # 1. INITIALIZE GLOBAL LIFETIME COUNTER
        if 'total_shots_fired' not in self.__dict__:
            self.total_shots_fired = 0

        # 2. THE ABSOLUTE RATE LIMIT GUARD
        # We stop at 48 to be safe. If we hit 50, the connection drops.
        if self.total_shots_fired >= 44:
            return None

        # 3. THE "SPACED OUT" THROTTLE
        # Firing every 400 steps allows 48 shots to last 19,200 steps.
        if self.current_step % 400 != 0:
            return None

        # 4. CHOOSE SIDE & PRICE
        qty = 100
        # Placing orders 1% to 3% away to catch the "Stressed" volatility spikes
        offset = mid * random.uniform(0.01, 0.03)
        
        # Logic to choose side based on inventory or randomness
        ...
        # print(f"--- Chaos Shot {self.total_shots_fired}/48 at step {self.current_step} ---")
    ...
    def _strategy_hft_dominated(self, bid: float, ask: float, mid: float, regime: str) -> Optional[Dict]:
        """
        High-Frequency: Fast reaction to small price gaps.
        Goal: Rapid turnover.
        """
        # HFT markets have thin spreads. We want to be at the front of the queue.
        # Use a very small tick to outbid others
        qty = 200 
        
        # Simple Mean Reversion: If price ticks up, it'll likely tick back down
        momentum = self._recent_momentum()
        if momentum > 0.02:
            return self._create_order("SELL", ask, qty)
        elif momentum < -0.02:
        ...
        return self._create_order("BUY", bid, 100) # Default small bid
