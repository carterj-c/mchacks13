...
class TradingBot:
    ...
    def __init__(
        ...
        self.trade_log_path = os.path.join(
            os.getcwd(), f"received_trades_{self.student_id}_{self.scenario}.csv"
        )
        self._trade_log_file: Optional[Any] = None
        self._trade_csv_writer = None
    ...
    def _on_market_data(self, ws, message: str):
        try:
            ...
            self._log_trade_snapshot(
                self.current_step, self.last_bid, self.last_ask, self.last_mid, recv_time
            )
    ...
    def _init_trade_log(self) -> None:
        try:
            self._trade_log_file = open(self.trade_log_path, "a", newline="")
            self._trade_csv_writer = csv.writer(self._trade_log_file)
            if self._trade_log_file.tell() == 0:
                self._trade_csv_writer.writerow(
                    ["step", "bid", "ask", "mid", "spread", "scenario", "timestamp"]
                )
                self._trade_log_file.flush()
        except Exception as exc:
            print(f"[{self.student_id}] Trade log init failed: {exc}")
            self._trade_log_file = None
            self._trade_csv_writer = None

    def _log_trade_snapshot(self, step: int, bid: float, ask: float, mid: float, timestamp: float) -> None:
        if not self._trade_csv_writer:
            return
        spread = ask - bid
        try:
            self._trade_csv_writer.writerow(
                [step, bid, ask, mid, spread, self.scenario, timestamp]
            )
            if self._trade_log_file:
                self._trade_log_file.flush()
        except Exception as exc:
            print(f"[{self.student_id}] Trade logging failed: {exc}")
